package io.renren.modules.sys.service;

import com.baomidou.mybatisplus.extension.service.IService;
import io.renren.common.utils.PageUtils;
import io.renren.modules.sys.entity.NideshopCategoryEntity;
import io.renren.modules.sys.entity.NideshopGoodsEntity;

import java.util.List;
import java.util.Map;

/**
 * 
 *
 * @author Mark
 * @email sunlightcs@gmail.com
 * @date 2020-06-29 17:44:54
 */
public interface NideshopGoodsService extends IService<NideshopGoodsEntity> {

    PageUtils queryPage(Map<String, Object> params);
    List<NideshopGoodsEntity> queryHotGoods();
    String queryGoodsPicUrl(int goodid);
    PageUtils listByCataLog(Map<String, Object> params,int catalogid);
    PageUtils queryPageByCataLogs(Map<String, Object> params,List<Integer> catalogs);
}

